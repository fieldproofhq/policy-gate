...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && (url.pathname === '/llms.txt' || url.pathname === '/.well-known/llms.txt')) {
      ...
      const body = [
        // This is the first thing an LLM agent reads. It used to be a bare list of payment
        // links that opened with a $42 checkout for a different product and never said what
        // the service does. Lead with the free surfaces: evaluate-before-paying is the whole
        // argument, and it cannot work if the free half goes unmentioned.
        '# Fieldproof Policy Gate',
        '',
        '> Deterministic allow / require_approval / deny verdicts for a proposed agent action,',
        '> returned BEFORE the action happens. No model in the enforcement path: same input,',
        '> same verdict, with the matched rule and rationale so it is auditable afterwards.',
        '',
        '## Free, and complete - evaluate before paying anything',
        '',
        `Worked verdicts incl. denials: ${url.origin}/v1/example`,
        `Every rule, condition and rationale: ${url.origin}/v1/policies`,
        `The seven ethics canons: ${url.origin}/v1/canons`,
        'MIT implementation, zero dependencies: https://github.com/fieldproofhq/policy-gate',
        '',
        'Nothing about how a verdict is reached sits behind the paywall. You pay for evaluation',
        'of YOUR policy against YOUR action, not for access to ours.',
        '',
        '## Paid',
        '',
        `POST ${url.origin}/v1/check - $0.005 per verdict, x402 USDC on Base.`,
        '  Optional `ledger` bounds cumulative exposure: a per-action gate cannot see repetition,',
        '  and 49 payments of $40 pass a $50 rule individually. Omit it where a policy declares a',
        '  cumulative rule and the verdict is deny with ledger_required.',
        `POST ${url.origin}/v1/ethics-check - $0.01. Screens a DECLARED action against seven canons.`,
        '  It screens the declaration, not the world; it cannot see what you do not declare.',
        `POST ${url.origin}/v1/sponsor - 42 USDC, one settlement that meets our first-income bar.`,
        `MCP over streamable HTTP: ${url.origin}/mcp`,
        '',
        '## Other rails',
        '',
        `All rails as data: ${url.origin}/v1/pay (send Accept: application/json)`,
        'The $42 Governance Pack: https://store.3labs.io/l/agentic-ai-governance-pack',
        '  One of its seven documents is free and complete:',
        '  https://github.com/fieldproofhq/policy-gate/blob/main/guides/mcp-tool-integration-security-checklist.md',
        'Card checkout: https://buy.stripe.com/eVq4gA91U3Rr1Yt6z31sQ00',
        '  NOTE: that checkout delivers the Ethics Check and C-suite Word kits, NOT the',
        '  Governance Pack. For the pack use the store link above; its page takes cards too.',
        '',
        '## Honest status',
        '',
        'External revenue: $0.00. The wallet holds 0.005 USDC from our own Aug 10 smoke test,',
        'which is excluded. No stranger has paid for anything yet, so the settle path has been',
        'exercised against the facilitator with a signed probe but never with real money.',
        '',
...
<p>This page stays here so you can pick a rail. Land a fractional CMO practice in 30 days — or load the same kit so an agent can hold the seat. The file is attached on Gumroad and downloads after payment. Generic card checkout is a different product.</p>
...
${cardFallbackHtml()}
<p>Agent contract: <a href="https://fieldproofhq.github.io/csuite/cmo/">fieldproofhq.github.io/csuite/cmo</a>. Scan: <a href="${url.origin}/v1/pay/scan">USDC / BTC / Zelle</a>.</p>
</body></html>`;
      return new Response(html, { status: 200, headers: { 'content-type': 'text/html; charset=utf-8', Link: paymentLinkHeader(), ...corsHeaders() } });
    }
...
<p>This page stays here so you can pick a rail. Listed at <strong>$42</strong>. Pay more if you want. No Fieldproof account. Generic card checkout is a different product.</p>
...
${cardFallbackHtml()}
<p>Store: <a href="${checkout}">store.3labs.io/l/tip-jar</a>. Scan: <a href="${url.origin}/v1/pay/scan">USDC / BTC / Zelle</a>.</p>
</body></html>`;
      return new Response(html, { status: 200, headers: { 'content-type': 'text/html; charset=utf-8', Link: paymentLinkHeader(), ...corsHeaders() } });
    }
