...
assert.match(cardPage, /http-equiv="refresh"/);
assert.match(cardPage, /location\.replace/);
assert.match(res.headers.get('link') || '', /rel="payment"/);
res = await call(freeEnv, 'GET', '/pay');
assert.strictEqual(res.status, 302);
assert.equal(res.headers.get('location'), 'https://buy.stripe.com/eVq4gA91U3Rr1Yt6z31sQ00');
res = await call(freeEnv, 'GET', '/pay', undefined, { accept: 'text/html' });
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /text\/html/);
const shortPayPage = await res.text();
assert.match(shortPayPage, /Pay \$42 with card/);
assert.match(shortPayPage, /buy\.stripe\.com\/eVq4gA91U3Rr1Yt6z31sQ00/);
assert.match(shortPayPage, /http-equiv="refresh"/);
assert.match(shortPayPage, /location\.replace/);
assert.match(res.headers.get('link') || '', /rel="payment"/);
res = await call(freeEnv, 'GET', '/pay/', undefined, { accept: 'text/html' });
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /text\/html/);
...
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /text\/html/);
const freeOfferPage = await res.text();
assert.match(freeOfferPage, /Pay \$42 with card/);
assert.match(freeOfferPage, /buy\.stripe\.com\/eVq4gA91U3Rr1Yt6z31sQ00/);
assert.match(freeOfferPage, /http-equiv="refresh"/);
assert.match(freeOfferPage, /location\.replace/);
...
assert.match(payDoc.also.did, /\/\.well-known\/did\.json$/);
res = await call(freeEnv, 'GET', '/.well-known/pay', undefined, { accept: 'text/html' });
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /text\/html/);
const wellKnownPayPage = await res.text();
assert.match(wellKnownPayPage, /Pay \$42 with card/);
assert.match(wellKnownPayPage, /buy\.stripe\.com\/eVq4gA91U3Rr1Yt6z31sQ00/);
assert.match(wellKnownPayPage, /http-equiv="refresh"/);
assert.match(wellKnownPayPage, /location\.replace/);
assert.match(res.headers.get('link') || '', /rel="payment"/);
res = await call(freeEnv, 'GET', '/.well-known/pay.json', undefined, { accept: 'text/html' });
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /application\/json/);
assert.equal((await res.json()).price_usd, 42);
res = await call(freeEnv, 'GET', '/.well-known/did.json');
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type') || '', /application\/did\+json/);
const didDoc = await res.json();
assert.equal(didDoc.id, 'did:web:policy-gate.example.workers.dev');
...
assert.ok(didDoc.service.some((svc) => svc.id.endsWith('#invoice') && /\/v1\/invoice$/.test(svc.serviceEndpoint)));
assert.match(res.headers.get('link') || '', /rel="payment"/);
res = await call(freeEnv, 'GET', '/sitemap.xml');
assert.match(await res.text(), /\/\.well-known\/did\.json/);
res = await call(freeEnv, 'GET', '/llms-full.txt');
assert.match(await res.text(), /did\.json/);
...
assert.match(homeHtml, /http-equiv="refresh"/);
assert.match(homeHtml, /location\.replace/);
...
assert.match(payIndex, /Pay \$42 with card/);
assert.match(payIndex, /http-equiv="refresh"/);
assert.match(payIndex, /location\.replace/);
...
res = await call(freeEnv, 'GET', '/v1/checkouts', undefined, { accept: 'text/html' });
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /text\/html/);
const checkoutsPage = await res.text();
assert.match(checkoutsPage, /Pay \$42 with card/);
assert.match(checkoutsPage, /buy\.stripe\.com\/eVq4gA91U3Rr1Yt6z31sQ00/);
assert.match(checkoutsPage, /http-equiv="refresh"/);
assert.match(checkoutsPage, /location\.replace/);
assert.match(res.headers.get('link') || '', /rel="payment"/);
...
assert.match(x402Page, /Pay 42 USDC in this browser/);
assert.match(x402Page, /eth_sendTransaction/);
...
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /text\/html/);
const offerPage = await res.text();
assert.match(offerPage, /Pay \$42 with card/);
assert.match(offerPage, /buy\.stripe\.com\/eVq4gA91U3Rr1Yt6z31sQ00/);
assert.match(offerPage, /http-equiv="refresh"/);
assert.match(offerPage, /location\.replace/);
...
assert.match(zellePage, /Open \$42 Zelle memo/);
assert.match(zellePage, /http-equiv="refresh"/);
assert.match(zellePage, /location\.replace\(PAY_URI\)/);
assert.doesNotMatch(zellePage, /Mobi\|Android\|iPhone\|iPad/);
assert.match(res.headers.get('link') || '', /rel="payment"/);
...
assert.match(btcPage, /http-equiv="refresh"/);
assert.match(btcPage, /location\.replace\(PAY_URI\)/);
assert.doesNotMatch(btcPage, /Mobi\|Android\|iPhone\|iPad/);
assert.match(res.headers.get('link') || '', /rel="payment"/);
...
res = await call(freeEnv, 'GET', '/package.json', undefined, { accept: 'text/html' });
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /text\/html/);
const npmPkgPage = await res.text();
assert.match(npmPkgPage, /Pay \$42 with card/);
assert.match(npmPkgPage, /buy\.stripe\.com\/eVq4gA91U3Rr1Yt6z31sQ00/);
assert.match(npmPkgPage, /http-equiv="refresh"/);
assert.match(npmPkgPage, /location\.replace/);
assert.match(res.headers.get('link') || '', /rel="payment"/);
res = await call(freeEnv, 'GET', '/');
...
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /text\/html/);
const quotePage = await res.text();
assert.match(quotePage, /Pay \$42 with card/);
assert.match(quotePage, /buy\.stripe\.com\/eVq4gA91U3Rr1Yt6z31sQ00/);
assert.match(quotePage, /http-equiv="refresh"/);
assert.match(quotePage, /location\.replace/);
res = await call(freeEnv, 'GET', '/openapi.json');
...
res = await call({ PAY_TO: '0x07C2383008a9ed30581f27Db5531E19411c94fb3' }, 'GET', '/v1/pay/usdc.png');
assert.strictEqual(res.status, 302);
const usdcQrLoc = res.headers.get('location') || '';
assert.match(usdcQrLoc, /qrserver\.com\/v1\/create-qr-code/);
assert.match(usdcQrLoc, /uint256%3D42000000|uint256=42000000/);
assert.match(usdcQrLoc, /0x07C2383008a9ed30581f27Db5531E19411c94fb3/);
res = await call({ PAY_TO: '0x07C2383008a9ed30581f27Db5531E19411c94fb3' }, 'GET', '/v1/pay/usdc.qr');
assert.strictEqual(res.status, 302);
assert.match(res.headers.get('location') || '', /8453/);
res = await call(freeEnv, 'GET', '/openapi.json');
assert.ok((await res.json()).paths['/v1/pay/usdc.png'].get);
const listedUsdcQr = listed.checkouts.find((o) => o.id === 'usdc-qr');
assert.equal(listedUsdcQr.amount_usd, 42);
assert.match(listedUsdcQr.url, /\/v1\/pay\/usdc\.png$/);
res = await call(freeEnv, 'GET', '/llms-full.txt');
assert.match(await res.text(), /\/v1\/pay\/usdc\.png/);
res = await call(freeEnv, 'GET', '/sitemap.xml');
assert.match(await res.text(), /\/v1\/pay\/usdc\.png/);
assert.match(payManifest.also.usdc_qr, /\/v1\/pay\/usdc\.png$/);
res = await call(freeEnv, 'GET', '/.well-known/host-meta.json');
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /application\/jrd\+json/);
assert.match(res.headers.get('link') || '', /rel="payment"/);
const hostMetaJsonDoc = await res.json();
assert.ok(hostMetaJsonDoc.links.some((link) => link.rel === 'payment' && link.href === 'https://buy.stripe.com/eVq4gA91U3Rr1Yt6z31sQ00'));
assert.ok(hostMetaJsonDoc.links.some((link) => link.rel === 'lrdd' && /webfinger\?resource=\{uri\}/.test(link.template)));
assert.ok(hostMetaJsonDoc.links.some((link) => /\/v1\/invoice$/.test(link.href)));
assert.equal(hostMetaJsonDoc.properties['http://schema.org/price'], '42.00');
res = await call(freeEnv, 'GET', '/openapi.json');
assert.ok((await res.json()).paths['/.well-known/host-meta.json'].get);
res = await call(freeEnv, 'GET', '/llms-full.txt');
assert.match(await res.text(), /host-meta\.json/);
res = await call(freeEnv, 'GET', '/sitemap.xml');
assert.match(await res.text(), /\/\.well-known\/host-meta\.json/);
assert.match(payManifest.also.host_meta, /\/\.well-known\/host-meta\.json$/);
res = await call(freeEnv, 'GET', '/llms-full.txt');
assert.match(await res.text(), /\/v1\/pay\/usdc\.uri/);
...
assert.match(res.headers.get('link') || '', /rel="payment"/);
res = await call(paidEnv, 'GET', '/mcp', undefined, { accept: 'text/html' });
assert.strictEqual(res.status, 200);
assert.match(res.headers.get('content-type'), /text\/html/);
const mcpPage = await res.text();
assert.match(mcpPage, /Pay \$42 with card/);
assert.match(mcpPage, /buy\.stripe\.com\/eVq4gA91U3Rr1Yt6z31sQ00/);
assert.match(mcpPage, /http-equiv="refresh"/);
assert.match(mcpPage, /location\.replace/);
assert.match(res.headers.get('link') || '', /rel="payment"/);
