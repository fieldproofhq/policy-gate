...
assert.strictEqual(res.status, 200, 'GET /v1/sponsor without JSON Accept is the HTML checkout');
assert.match(res.headers.get('content-type'), /text\/html/);
const defaultSponsorPage = await res.text();
assert.match(defaultSponsorPage, /42 USDC/);
assert.match(defaultSponsorPage, /Copy address/);
assert.match(defaultSponsorPage, /Pay 42 USDC in this browser/);
assert.match(defaultSponsorPage, /eth_sendTransaction/);
assert.match(defaultSponsorPage, /0xa9059cbb/);
assert.match(defaultSponsorPage, /wallet_switchEthereumChain/);
...
assert.strictEqual(res.status, 200);
const mcpWellKnown = await res.json();
assert.match(mcpWellKnown.server_url, /\/mcp$/);
assert.match(mcpWellKnown.x402, /\/\.well-known\/x402$/);
res = await call(paidEnv, 'GET', '/v1/pay/pack');
assert.strictEqual(res.status, 200);
