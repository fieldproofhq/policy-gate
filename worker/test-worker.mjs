...
assert.match(homeHtml, /\$42 tip jar/);
...
assert.match(payIndex, /Browse the \$42 store/);
...
assert.strictEqual(res.status, 402, 'unpaid GET /v1/sponsor must quote, not look like a free 200');
...
assert.ok(res.headers.get('payment-required'), 'GET carries the v2 PAYMENT-REQUIRED header');
